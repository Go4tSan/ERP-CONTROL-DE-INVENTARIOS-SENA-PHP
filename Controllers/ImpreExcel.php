...
class ImpreExcel extends Controller{
...
public function excel()
{
    ...
    $data = $this->model->getImpresoraE();
    header('Content-type: application/vnd.ms-excel');
header('Content-Disposition: attachment; filename=Reporte_Impresoras_'.$Fecha_Reporte.'.xls');
header('Pragma: no-cache');
header('Expires: 0');

echo'<table border=1>';
echo'<tr>';
echo'<th colspan=14>Reporte de Impresoras</th>';
echo'</tr>';
echo'<tr><th>ID</th><th>IP</th><th>Modelo</th><th>Ambiente</th><th>Piso</th><th>Serial</th><th>Placa</th><th>Foto del objeto</th><th>Foto del ambiente</th><th>Marca</th><th>Observacion</th><th>Fecha de la observacion</th><th>MAC</th><th>Estado</th></tr>';
for( $i=0; $i < count($data); $i++) {
 echo '<tr>';
 echo '<td>'.$data[$i]['Id_Impresora'].'</td>';
